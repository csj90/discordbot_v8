...
async function validateConfig(config) {
	...
	if (config.api_keys.masterToken) {
		try {
			await require('axios').get('https://api2.csjgaming.com/api/info/validate', {
			...
			switch (e.response.status) {
				case 403:
					logger.error(`${chalk.red('✗')} Master API token is incorrect. Get it here: https://api2.csjgaming.com`);
					return true;
				case 502:
